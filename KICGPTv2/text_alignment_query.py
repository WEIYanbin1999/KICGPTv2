...
class Solver:
    ...
    def load_ent_to_text(self):
        with open('datasets/' + args.dataset + '/entity2text.txt', 'r') as file:
            entity_lines = file.readlines()
            for line in entity_lines:
                ent, text = line.strip().split(\"\\t\")\n                self.ent2text[ent] = text
    def load_rel_to_text(self):
        with open('datasets/' + args.dataset + '/relation2text.txt', 'r') as file:\n            rel_lines = file.readlines()
...
def parse_args():
    ...
    parser.add_argument('--output_path', default="./datasets/wn18rr/alignment/alignment_output.txt")
    parser.add_argument('--chat_log_path', default="./datasets/wn18rr/alignment/alignment_chat.txt")
...
if __name__ == '__main__':
    ...
    with open("datasets/" + args.dataset + "/demonstration/all_r_triples.txt", "r") as f:
        demonstration_r = json.load(f)

    if args.num_process == 1:
        main(args, demonstration_r, idx=-1, api_key=args.api_key)
    else:
        # Multi-process support for relation alignment
        import multiprocessing as mp
        relation_list = list(demonstration_r.keys())
        num_each_split = int(len(relation_list) / args.num_process)
        p = mp.Pool(args.num_process)
        
        for idx in range(args.num_process):
            start = idx * num_each_split
            if idx == args.num_process - 1:
                end = max((idx + 1) * num_each_split, len(relation_list))
            else:
                end = (idx + 1) * num_each_split
            split_relations = {r: demonstration_r[r] for r in relation_list[start:end]}
            try:
                p.apply_async(main, args=(args, split_relations, idx, all_keys[idx]))
            except Exception as e:
        ...
        # Merge split files into single alignment_output.txt
        print("Merging alignment output files...")
        ...
        # Merge chat logs
        print("Merging chat log files...")
        with open(args.chat_log_path, 'w') as outfile:
            for idx in range(args.num_process):
                idx_str = "0" + str(idx) if idx < 10 else str(idx)
                split_file = args.chat_log_path + "_" + idx_str
                if os.path.exists(split_file):
                    with open(split_file, 'r') as infile:
                        outfile.write(infile.read())
                    os.remove(split_file)
        
        print(f"All alignment results saved to {args.output_path}")